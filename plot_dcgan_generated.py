...
def setup_logger() -> logging.Logger:
    ...
    return logging.getLogger("plot_dcgan_generated")


def load_generated(npz_path: Path) -> tuple[np.ndarray, np.ndarray, list[str]]:
    if not npz_path.exists():
        raise FileNotFoundError(f"Missing generated npz: {npz_path}. Please run python generate_dcgan.py first.")
...
def plot_class_samples(
    ...
    axes[0].set_title(f"DCGAN generated {class_name} - time domain")
    ...
    axes[1].set_title(f"DCGAN generated {class_name} - frequency domain")
...
def main(config: dict = CONFIG) -> None:
    ...
    logger.info("Saved DCGAN preview plots to %s", out_dir)
