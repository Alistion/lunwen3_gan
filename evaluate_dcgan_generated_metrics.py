...
def setup_logger() -> logging.Logger:
    ...
    return logging.getLogger("evaluate_dcgan_generated_metrics")
...
def plot_one_class_comparison(
    ...
    for i, (generated, generated_index) in enumerate(zip(generated_samples, generated_indices)):
        axes[0].plot(
            ...
            linewidth=0.85,
            alpha=0.45 + 0.15 * i,
            label=f"DCGAN generated #{int(generated_index)}",
        )
    ...
    fig.tight_layout()
    fig.savefig(out_dir / f"{label}_{class_name}_real_vs_dcgan_time_freq.png", dpi=220)
    plt.close(fig)
...
def save_class_sample_plots(
    ...
    for class_name in CLASS_NAMES:
        ...
        plot_path = (plot_dir / f"{label}_{class_name}_real_vs_dcgan_time_freq.png").as_posix()
...
def main(config: dict = CONFIG) -> None:
    ...
    x_gen, y_gen = load_xy(generated_npz)
    ...
    plot_df.to_csv(out_dir / "dcgan_sample_plot_indices.csv", index=False)
    ...
    pair_df.to_csv(out_dir / "dcgan_pair_metrics.csv", index=False)
    ...
    summary.to_csv(out_dir / "dcgan_generated_quality_metrics.csv", index=False)

    overall = overall_row(summary)
    if overall:
        pd.DataFrame([overall]).to_csv(out_dir / "dcgan_overall_metrics.csv", index=False)

    logger.info("Saved pair metrics to %s", out_dir / "dcgan_pair_metrics.csv")
    logger.info("Saved class metrics with CI to %s", out_dir / "dcgan_generated_quality_metrics.csv")
    logger.info("Saved sample time/frequency plots to %s", out_dir / "sample_time_freq")
    if overall:
        logger.info("Saved overall metrics to %s", out_dir / "dcgan_overall_metrics.csv")
