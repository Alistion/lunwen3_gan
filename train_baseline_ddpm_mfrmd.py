...
CONFIG = {
    ...
    "run_root": "runs/baseline_ddpm_mfrmd_v1",
    "run_id": None,
...
def setup_logger() -> logging.Logger:
    ...
    return logging.getLogger("train_baseline_ddpm_mfrmd")
...
def select_indices_by_count(
    ...
    for label, class_name in enumerate(class_names):
        ...
        rng.shuffle(idx)
        need = len(idx) if train_counts is None else int(train_counts.get(class_name, len(idx)))
        if need > len(idx):
    ...
    torch.save(
        {
            ...
            "model_kwargs": model_kwargs(config, len(class_names)),
...
def train(config: dict, out_dir: Path, logger: logging.Logger) -> tuple[Path, list[str]]:
    ...
    model = ConditionalUNet1D(**model_kwargs(config, len(class_names))).to(device)
    ...
    best_loss = float("inf")
    logger.info("Training MFRMD baseline DDPM on %d samples with device=%s", len(dataset), device)
    ...
    with log_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for epoch in range(1, int(config["epochs"]) + 1):
            model.train()
            losses = []
            pbar = tqdm(dataloader, desc=f"mfrmd-baseline epoch {epoch:03d}", leave=False)
            for x0, labels in pbar:
                ...
                pred_noise = model(scheduler.q_sample(x0, timesteps, noise), timesteps, labels)
...
def load_model(ckpt_path: Path, device: torch.device) -> tuple[ConditionalUNet1D, DDPMScheduler1D, dict]:
    payload = torch.load(ckpt_path, map_location=device)
    model = ConditionalUNet1D(**payload["model_kwargs"]).to(device)
...
def generate(config: dict, out_dir: Path, ckpt_path: Path, logger: logging.Logger) -> Path:
    ...
    with torch.no_grad():
        for label, class_name in enumerate(class_names):
            logger.info("Generating %d MFRMD baseline samples for %s", samples_per_class, class_name)
...
def evaluate(config: dict, out_dir: Path, logger: logging.Logger) -> None:
    ...
    pair_df.to_csv(eval_dir / "mfrmd_baseline_pair_metrics.csv", index=False)
    summary = (
    ...
    summary.to_csv(eval_dir / "mfrmd_baseline_generated_quality_metrics.csv", index=False)
    if not summary.empty:
        ...
        pd.DataFrame([overall]).to_csv(eval_dir / "mfrmd_baseline_overall_metrics.csv", index=False)
    logger.info("Saved MFRMD baseline evaluation metrics to %s", eval_dir)
...
def plot_samples(config: dict, out_dir: Path, logger: logging.Logger) -> None:
    ...
    for label, class_name in enumerate(class_names):
        ...
        for i, (ri, gi) in enumerate(zip(real_pick, gen_pick)):
            out_path = plot_dir / f"{class_name}_{i:02d}_real_vs_baseline_time_freq.png"
            plot_time_freq_pair(
                x_real[int(ri)],
                x_gen[int(gi)],
                int(config["fs"]),
                f"{class_name} baseline sample {i}",
                out_path,
    ...
    pd.DataFrame(rows).to_csv(out_dir / "plots" / "mfrmd_baseline_sample_plot_indices.csv", index=False)
    logger.info("Saved MFRMD baseline sample plots to %s", plot_dir)
